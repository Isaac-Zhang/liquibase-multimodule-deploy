...
--changeSet proc:Initial-tSQLt-ApplyConstraint-0 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:false
IF OBJECT_ID('tSQLt.ApplyConstraint', 'P') IS NULL EXEC('CREATE PROCEDURE [tSQLt].[ApplyConstraint] @TableName nvarchar(MAX),@ConstraintName nvarchar(MAX),@SchemaName nvarchar(MAX) AS BEGIN SELECT ret = 1 END')
GO



--changeSet proc:Initial-tSQLt-ApplyConstraint-1 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:true
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
ALTER PROCEDURE [tSQLt].[ApplyConstraint]
