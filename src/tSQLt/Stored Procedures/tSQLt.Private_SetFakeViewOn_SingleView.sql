...
--changeSet proc:Initial-tSQLt-Private_SetFakeViewOn_SingleView-0 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:false
IF OBJECT_ID('tSQLt.Private_SetFakeViewOn_SingleView', 'P') IS NULL EXEC('CREATE PROCEDURE [tSQLt].[Private_SetFakeViewOn_SingleView] @ViewName nvarchar(MAX) AS BEGIN SELECT ret = 1 END')
GO



--changeSet proc:Initial-tSQLt-Private_SetFakeViewOn_SingleView-1 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:true
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
ALTER PROCEDURE [tSQLt].[Private_SetFakeViewOn_SingleView]
