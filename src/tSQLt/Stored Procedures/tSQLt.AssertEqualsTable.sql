...
--changeSet proc:Initial-tSQLt-AssertEqualsTable-0 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:false
IF OBJECT_ID('tSQLt.AssertEqualsTable', 'P') IS NULL EXEC('CREATE PROCEDURE [tSQLt].[AssertEqualsTable] @Expected nvarchar(MAX),@Actual nvarchar(MAX),@FailMsg nvarchar(MAX) AS BEGIN SELECT ret = 1 END')
GO



--changeSet proc:Initial-tSQLt-AssertEqualsTable-1 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:true
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
---BUILD+
ALTER PROCEDURE [tSQLt].[AssertEqualsTable]
