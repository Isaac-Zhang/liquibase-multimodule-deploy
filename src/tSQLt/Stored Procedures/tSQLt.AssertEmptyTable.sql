...
--changeSet proc:Initial-tSQLt-AssertEmptyTable-0 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:false
IF OBJECT_ID('tSQLt.AssertEmptyTable', 'P') IS NULL EXEC('CREATE PROCEDURE [tSQLt].[AssertEmptyTable] @TableName nvarchar(MAX) AS BEGIN SELECT ret = 1 END')
GO



--changeSet proc:Initial-tSQLt-AssertEmptyTable-1 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:true
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
---Build+
ALTER PROCEDURE [tSQLt].[AssertEmptyTable]
