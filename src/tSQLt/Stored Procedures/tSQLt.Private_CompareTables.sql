...
--changeSet proc:Initial-tSQLt-Private_CompareTables-0 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:false
IF OBJECT_ID('tSQLt.Private_CompareTables', 'P') IS NULL EXEC('CREATE PROCEDURE [tSQLt].[Private_CompareTables] @Expected nvarchar(MAX),@Actual nvarchar(MAX),@ResultTable nvarchar(MAX),@ColumnList nvarchar(MAX),@MatchIndicatorColumnName nvarchar(MAX) AS BEGIN SELECT ret = 1 END')
GO



--changeSet proc:Initial-tSQLt-Private_CompareTables-1 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:true
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
---BUILD+
ALTER PROCEDURE [tSQLt].[Private_CompareTables]
