...
--changeSet proc:Initial-tSQLt-Private_CreateProcedureSpy-0 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:false
IF OBJECT_ID('tSQLt.Private_CreateProcedureSpy', 'P') IS NULL EXEC('CREATE PROCEDURE [tSQLt].[Private_CreateProcedureSpy] @ProcedureObjectId int,@OriginalProcedureName nvarchar(MAX),@LogTableName nvarchar(MAX),@CommandToExecute nvarchar(MAX) AS BEGIN SELECT ret = 1 END')
GO



--changeSet proc:Initial-tSQLt-Private_CreateProcedureSpy-1 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:true
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
---Build+
ALTER PROCEDURE [tSQLt].[Private_CreateProcedureSpy]
