...
--changeSet func:Initial-tSQLt-Private_GetForeignKeyDefinition-0 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:false
IF OBJECT_ID('tSQLt.Private_GetForeignKeyDefinition', 'IF') IS NULL EXEC('CREATE FUNCTION [tSQLt].[Private_GetForeignKeyDefinition](@SchemaName nvarchar(MAX),@ParentTableName nvarchar(MAX),@ForeignKeyName nvarchar(MAX)) RETURNS TABLE AS RETURN (SELECT ret = 1)')
GO



--changeSet func:Initial-tSQLt-Private_GetForeignKeyDefinition-1 endDelimiter:\nGO splitStatements:true stripComments:false runOnChange:true
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
ALTER FUNCTION  [tSQLt].[Private_GetForeignKeyDefinition](
